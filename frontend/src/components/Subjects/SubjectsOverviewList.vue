<template>
  <q-expansion-item icon="perm_identity" default-opened>
    <template v-slot:header>
      <div style="width: 100%">
        <span class="text-h5">Subjects</span>
      </div>
    </template>
    <q-table
          ...
          <q-btn-toggle
            ...
            @update:model-value="
              $q.localStorage.setItem(
                'configs_ui_subjectsDisplayMode',
                displayStyle
              )
            "
            :options="[
      ...
      <template v-slot:item="props">
        ...
        >
          <q-card
            ...
            @click="
              () => {
                $router.push(`/subjects/${props.row.id}`);
              }
            "
          >
...
import SubjectsNewDialog from './SubjectsNewDialog.vue';
import SubjectsEditDialog from './SubjectsEditDialog.vue';
...
let displayStyle = ref(
  $q.localStorage.getItem('configs_ui_subjectsDisplayMode') || 'grid'
);

function openNewSubjectDialog() {
  $q.dialog({
    component: SubjectsNewDialog,
    componentProps: {},
  });
}

function openDeleteDialog(data: Subject) {
  $q.dialog({
...
function openTagsDialog(subjectId: string, data: string[]) {
  $q.dialog({
    ...
    subjectTagsUpdate({ id: subjectId, tags });
  });
}
