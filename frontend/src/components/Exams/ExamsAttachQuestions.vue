...
import { getExamQuestions, putExamQuestions } from 'src/api/exams';
...
const { mutate: examUpdate } = useMutation({
  mutationFn: (data: { questions: string[] }) =>
    putExamQuestions(props.examId, data.questions),
  onSuccess: () => {
    //Replace with update queries
    queryClient.invalidateQueries({
      queryKey: ['questions', props.examId],
    });
  },
});

const { data: examQuestionsResponse } = useQuery({
  queryFn: () => getExamQuestions(props.examId),
  queryKey: ['questions', props.examId],
});

const examQuestionsData = computed(() => {
  const newData = examQuestionsResponse.value;
...
const questionIds = ref([
  ...(examQuestionsData.value?.map((question) => question.id) || []),
]);

function onApplyClick() {
  examUpdate({
    questions: questionIds.value,
  });
  onDialogOK();
}
